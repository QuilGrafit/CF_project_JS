...
export class HoroscopeGenerator {
    ...
    static getZodiacSign(birthDate) {
        ...
        if ((month === 2 && day >= 19) || (month === 3 && day <= 20)) return "pisces"; // Corrected this line
    ...
    static generate(userId, userData) {
        ...
        const seedStr = `<span class="math-inline">\{userId\}\_</span>{today.getFullYear()}<span class="math-inline">\{today\.getMonth\(\)\}</span>{today.getDate()}_${signKey}`;
        let seed = 0;
